...
function displaySearchResults(results, query) {
  ...
  for (var i = 0; i < allSections.length; i++) {
    if (allSections[i].id !== 'searchResults') {
...
function createSearchResultsSection() {
  ...
  var searchResultsDiv = document.createElement('section');
  searchResultsDiv.id = 'searchResults';
  searchResultsDiv.className = 'product-category';
  searchResultsDiv.style.display = 'none';
  searchResultsDiv.innerHTML = '<h2 id="searchResultsCount">Search Results</h2><div class="product-grid" id="searchResultsGrid"></div>';
  
  if (searchSection && searchSection.nextSibling) {
    productsMain.insertBefore(searchResultsDiv, searchSection.nextSibling);
  } else if (categoryNav && categoryNav.nextSibling) {
    productsMain.insertBefore(searchResultsDiv, categoryNav.nextSibling);
  } else {
    productsMain.appendChild(searchResultsDiv);
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  console.log('✅ Email validation active on all forms!');
  console.log('✅ Search feature fully functional!');
  console.log('✅ All products can be added to cart!');
});